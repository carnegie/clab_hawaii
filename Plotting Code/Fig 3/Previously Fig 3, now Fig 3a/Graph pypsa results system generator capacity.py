...
p5 = ax1.bar(ind, data5, width, bottom=data1+data2, color=pgp_q, hatch='//')
p6 = ax1.bar(ind, data6, width, bottom=data1+data2+data5, color=pgp_q, hatch='\\\\')

plt.ylabel('System Generation Capacity\n(GW)')
...
ax.set_title('Oahu Renewable System\nGeneration Capacity', y=1.01)
...
y = ['Solar', 'Wind','Electrolyzer', 'Fuel Cell']
#legend = plt.legend(reversed([p1, p2, p5, p6]), reversed(y), loc="best", bbox_to_anchor=(1, 0.7), fontsize='12', edgecolor='black')

plt.xticks(ha='center')
for handle in legend.legendHandles:
    handle.set_edgecolor('black')

for bar in p1 + p2 + p5 + p6:
    bar.set_linewidth(1)
    bar.set_edgecolor('black')

plt.savefig('C:\\Users\\Dominic\\desktop\\Oahu Results\\Oahu Optimized Renewable System Generation Capacity.jpg', dpi = 300, bbox_inches='tight')
plt.show()
