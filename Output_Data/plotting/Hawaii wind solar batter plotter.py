...
def get_cost_contributions(base):
    ...
    try:
        ...
        dicw = next((sub for sub in input_tech if sub['tech_name'] == 'node_1_wind2'), None)
        wind_t = ((np.multiply(dicw["fixed_cost"], results_tech["node_1_wind1 capacity"]))+(np.multiply(dicw["fixed_cost"], results_tech["node_1_wind2 capacity"])))
    except:
        wind_t = 0
    
    try:
        dics = next((sub for sub in input_tech if sub['tech_name'] == 'node_1_solar1'), None)
        dics = next((sub for sub in input_tech if sub['tech_name'] == 'node_1_solar2'), None)
        solar_t = ((np.multiply(dics["fixed_cost"], results_tech["node_1_solar1 capacity"]))+(np.multiply(dicw["fixed_cost"], results_tech["node_1_solar2 capacity"])))
    except:
        solar_t = 0
...
ax.set_title('Mock Hawaii with Multiple Generators', y=1)
...
plt.savefig('mock hawaii pv wind battery.jpg', dpi = 300, bbox_inches='tight')
plt.show()